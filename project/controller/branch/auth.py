...
class BranchLogin:
    def on_get(self, req, res):
        try:
            ...
            db_admin = session.query(BranchOffice).filter_by(eid=eid).first()
            ...
            if db_admin:
                ...
                if valid_password:
                    ...
                    # logger.info('[BranchOffice Login] successfully logged in. New access token generated .')
                    ...
                    logger.error('[BranchOffice Login] Invalid password. ')
                    res.status = falcon.HTTP_201
                ...
                logger.error('[BranchOffice Login] Invalid Email --> ' + str(eid))
                res.status = falcon.HTTP_201
                res.body = json.dumps({ 'status' : status, 'message' : message })
                return
        except Exception as e:
            status = False
            message = "[BranchOffice Login] Something went wrong : " +  str(e)
    ...
    def on_post(self,req,res):
        try:
            try:
                ...
                logger.info("[AddBranchOffice] Data {} ".format(json_data))
                name = json_data['name']
                eid = json_data['eid']
            ...
            if db_admin:
                ...
                logger.info('[AddBranchOffice] Account already exists ' + str(eid))
                res.status = falcon.HTTP_201
                res.body = json.dumps({ 'status' : status, 'message' : message })
                return
            else:
                ...
                try:
                    ...
                    logger.info('[AddBranchOffice] Signup success')
                    res.status = falcon.HTTP_201
                    res.body = json.dumps({ 'status' : status, 'message' : message })
                    return
                    ...
                    status = False
                    message = "[AddBranchOffice] Error while adding admin : " +  str(e)
                    logger.error(message)
                    res.status = falcon.HTTP_200
                    res.body = json.dumps({'status': status, 'message': message })
                    return
            ...
            status = False
            message = "[AddBranchOffice] Something went wrong : " +  str(e)
            logger.error(message)
            res.status = falcon.HTTP_200
            res.body = json.dumps({'status': status, 'message': message })
